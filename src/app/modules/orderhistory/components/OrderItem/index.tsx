...
const OrderItem: React.FC<Props> = ({ item, showEdit, index }) => {
    ...
    return (
        <tr style={{margin:100}}>
            ...
            <td >
                {(item.viewend>0&&item.viewend!=null)&&<span style={{color:'white',fontSize:11,backgroundColor:item.viewend>=(item.vieworder)?"rgba(16,128,201,0.66)":"#b7080f"}} className='badge badge-success'>
                    {item.viewend}</span>}
                {

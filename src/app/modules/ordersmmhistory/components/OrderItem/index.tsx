...
const OrderItem: React.FC<Props> = ({ item, showEdit, index }) => {
    ...
    const clickDeleteHandler = () => {
        if (window.confirm("bạn có chắc chắn muốn xóa đơn : "+item.order_id!) == true) {
            dispatch(actions.deleteOrderRequest(item.order_id.toString()))
        }
    }
        ...
        <tr style={{margin:100,backgroundColor:item.checked==true?"rgba(252,226,207,0.62)":""}}>
            ...
            <td>
                ...
                <br/>
                    ...
                    }}><span style={{color:'#b7080f',fontWeight:"bold",fontSize:11,marginRight:5,marginBottom:5}} >{new Date(item.end_time).toLocaleDateString('vn-VN').replace("/2025","") +" "+new Date(item.end_time).toLocaleTimeString('vn-VN')}</span>
                    </span>
                    </span>
                    </span></span>}
                    <br/>
                    ...
                    <span style={{color:'white',fontSize:10,padding:2,backgroundColor:"rgba(222,58,58,0.97)",marginRight:5,marginBottom:5}} className='badge badge-success'>
                            {item.refund}
                        </span>
                    </span>
                    </span></span>}
